...
| A-S05 | Approve multiline clipboard/share text above 1,024 bytes, then repeat and cancel after a chunk boundary | Preview matches destination/layout and discloses chunk count/Shift+Enter; exact content is typed without boundary characters or automatic submission in the chosen disposable field; progress stays global; cancellation stops without later replay. Verify IME clipboard-panel line breaks also use Shift+Enter while physical/editor-action Enter remains plain Enter. |
