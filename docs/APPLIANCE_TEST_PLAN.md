...
- authority form (IP/mDNS), network topology, HTTPS trust mode, and redacted
  certificate fingerprint. Cleartext HTTP is permitted only for A-R13's explicit
  pre-authentication AP bootstrap;
...
| A-S02 | Direct H.264 for 10 minutes | First frame, stable FPS/size, no black preview; stall causes a fresh bounded reconnect. |
| A-S03 | Force/choose MJPEG with frame detection off, then explicitly enable it and restart MJPEG | Fresh fallback surface, correct size/touch mapping; enabled start sends one temporary pause and does not require opening the IME to reveal video. |
...
| A-R13 | Configure a disposable Wi-Fi target/AP bootstrap and exercise non-destructive Tailscale state transitions | Manual endpoint/SSID only, no scan, no authenticated cookie in AP traffic, secrets cleared, official HTTPS login handoff, and one dispatch per reviewed command. |
...
Retain at least one phone result on API 26, 29, 31, 33, 34, and 37 for the
clipboard/IME/lifecycle flow. On API 37 also cover portrait, landscape,
